...
int  CheckAlreadyExists(char NewInfo[100], char TypeExists);//check fault when add student
void PrintStudent(struct StudentInfo A);
void ShowAllStudent();//show all student have GPA from high to low
void ShowGPAHightoLow();//GPA from High to Low
int  SearchStudent();//searching via ID
void EditStudent();
void DeleteCourseIndex(int FirstCourseIndex);//use edit Student
void DeleteStudentIndex(int StudentIDdelete);//use edit Student
void DeleteStudent();
...
void ReadStudentFromFile(struct StudentInfo student[]) {
	char Filename[100];
	printf("Enter the link and name file : "); scanf(" %s", Filename);
...
int CheckAlreadyExists(char NewInfo[100], char TypeExists) {
...
void DeleteCourseIndex(int FisrtCourseIndex) {
...
void DeleteStudentIndex(int StudentIDdelete) {
